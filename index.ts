...
// node api :(
if (!(await import('fs')).existsSync('frames'))
    (await import('fs')).mkdirSync('frames');
else {
    // clean up
    (await import('fs')).rmdirSync('frames', { recursive: true });
    (await import('fs')).mkdirSync('frames');
}
...
for (let i = 0; i < audio.duration * FPS; i++) {
    ...
    await Bun.write(`frames/frame${i.toString().padStart(max_pad, '0')}.png`, canvas.createPNGStream().read());
}
console.log("Done!");
...
if (!options.no_video) {
    await Bun.spawn(['ffmpeg', '-y',
        '-r', `${FPS}`,
        '-i', `frames/frame%${max_pad.toString().padStart(2, '0')}d.png`,
        '-i', INPUT_FILE,
        '-c:v', `${VIDEO_CODEC}`,
        '-c:a', `${AUDIO_CODEC}`,
        '-r', `${FPS}`,
